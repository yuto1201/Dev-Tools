...
| 37 | ヘッダーアクション | `.app-header-actions` | エディタヘッダーの中央アクション領域（save/import/export/settings） |
| 38 | 保存ステータス | `.btn-save-done` `.btn-save-error` | ボタン内表示（保存中→保存済み→元に戻る） |
| 39 | 編集可能タイトル | `.app-title-editable` | エディタヘッダーのインライン編集可能タイトル |
| 40 | 同期状態ドット | `.sync-dot` | 同期状態ドット（ヘッダー右端） |
| 41 | Drive プログレス | `.drive-progress` | 画面上部の同期中アニメーションバー |

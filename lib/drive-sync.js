/* ═══ DRIVE SYNC ═══
   ...
   使い方（各ツールから）:
     ...
     <script src="../lib/drive-sync.js"></script>
...
// マルチパートアップロード上限（Google Drive API = 5MB）
const MULTIPART_LIMIT = 4 * 1024 * 1024; // 余裕を持って 4MB

function _buildUploadMeta(localKey, updatedAt, existingFileId){
...
// 小さいファイル用: マルチパートアップロード（5MB 以下）
async function _uploadMultipart(metadata, content, existingFileId){
...
// 大きいファイル用: Resumable アップロード（5MB 超）
async function _uploadResumable(metadata, content, existingFileId){
  const url = existingFileId
    ? `${API_UPLOAD}/${existingFileId}?uploadType=resumable&fields=id,appProperties`
    : `${API_UPLOAD}?uploadType=resumable&fields=id,appProperties`;
  const method = existingFileId ? 'PATCH' : 'POST';

  // Step 1: セッション開始 → Location ヘッダーに resumable URI が返る
  const initResp = await _apiFetch(url, {
    method,
    headers: {'Content-Type': 'application/json; charset=UTF-8'},
    body: JSON.stringify(metadata),
  });
  const resumableUri = initResp.headers.get('Location');
  if(!resumableUri) throw new Error('Resumable upload: Location ヘッダーなし');

  // Step 2: コンテンツを PUT（認証不要 — URI 自体がトークン）
  const uploadResp = await fetch(resumableUri, {
    method: 'PUT',
  ...
  if(!uploadResp.ok){
    ...
    throw new Error(`Resumable upload failed ${uploadResp.status}: ${errBody}`);
  }
  return await uploadResp.json();
}

async function _uploadFile(localKey, content, updatedAt, existingFileId){
  const metadata = _buildUploadMeta(localKey, updatedAt, existingFileId);
  if(content.length > MULTIPART_LIMIT){
    return _uploadResumable(metadata, content, existingFileId);
  }
  return _uploadMultipart(metadata, content, existingFileId);
}
...
async function fullSync(){
  ...
  try{
    ...
    for(const localKey of localKeys){
      ...
      if(drive){
        if(drive.updatedAt > localUpdatedAt){
          // Drive 側が新しい → ダウンロード + ローカルキャッシュ
          try{
            const content = await _downloadFile(drive.fileId);
            try{ localStorage.setItem(localKey, content); }catch(e){ try{ localStorage.removeItem(localKey); }catch(e2){} }
          ...
          // 同じ → fileId だけ記録（loadItem での Drive フォールバックに必要）
          _syncMeta[localKey] = Object.assign({}, _syncMeta[localKey], {fileId: drive.fileId});
        }
    ...
    // 2) 残った driveByKey は「Drive にだけある」キー → ダウンロード + ローカルキャッシュ
...
// ─── Storage abstraction ──────────────────────────────────
// ログイン時: Drive が一次ストレージ、localStorage はキャッシュ
// 未ログイン時: localStorage が一次ストレージ

async function saveItem(localKey, content){
  const ns = _findNamespace(localKey);
  if(!ns) throw new Error('未登録キー: ' + localKey);
  ...
  if(isSignedIn()){
    ...
    _loadSyncMeta();
    const fileId = _syncMeta[localKey] && _syncMeta[localKey].fileId;
    const result = await _uploadFile(localKey, content, updatedAt, fileId);
    _syncMeta[localKey] = {updatedAt, fileId: result && result.id};
    _saveSyncMeta();
    // localStorage にキャッシュ（失敗しても OK）
    try{ localStorage.setItem(localKey, content); }catch(e){ try{ localStorage.removeItem(localKey); }catch(e2){} }
    return;
  }
  // 未ログイン → localStorage（容量超過は呼び出し側に throw）
  localStorage.setItem(localKey, content);
}

async function loadItem(localKey){
  // まず localStorage キャッシュを試す（高速）
  const cached = localStorage.getItem(localKey);
  if(cached !== null) return cached;

  // キャッシュになければ Drive から取得
  if(isSignedIn()){
    _loadSyncMeta();
    let fileId = _syncMeta[localKey] && _syncMeta[localKey].fileId;
    if(!fileId){
      fileId = await _findFileIdByName(localKey + FILE_SUFFIX);
    }
    if(fileId){
      const content = await _downloadFile(fileId);
      // キャッシュに入れる（失敗しても OK）
      try{ localStorage.setItem(localKey, content); }catch(e){}
      _syncMeta[localKey] = Object.assign({}, _syncMeta[localKey], {fileId});
      _saveSyncMeta();
      return content;
    }
  }
...
async function removeItem(localKey){
  localStorage.removeItem(localKey);
  ...
  if(isSignedIn()){
    ...
    _saveSyncMeta();
    if(fileId){
      try{ await _deleteFile(fileId); }catch(e){ console.error('Drive delete failed:', e); }
    }
  } else {
    _loadSyncMeta();
    delete _syncMeta[localKey];
